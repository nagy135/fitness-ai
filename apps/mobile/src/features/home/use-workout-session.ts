...
export function useWorkoutSession(
  ...
  async function submitPrompt(prompt: string): Promise<boolean> {
    ...
    try {
      ...
      const result =
        mode === 'workout' ? await workoutAI({ prompt }) : await analysisAI({ prompt });
      setResponses((previous) => ({ ...previous, [mode]: result }));
      return true;
    } catch {
      setError(
        mode === 'workout'
          ? 'The request did not finish. Check your sets before resending; some changes may have been applied.'
